using ModelingToolkit 
using DifferentialEquations
using IfElse


@variables t
D = Differential(t)
@parameters TIME_STEP = 10
@parameters Output_in_1965 = 8.519e12
@parameters Red_Cost_Scale = 0.0686
@parameters A_UO_Heat_Cap = 44.248
@parameters one_year = 1.0
@parameters Climate_Sensitivity = 2.908
@parameters Atmos_Retention = 0.64
@parameters emissions_sensitivity = 0.0
@parameters init_Atmos_UOcean_Temp = 0.2
@parameters init_Deep_Ocean_Temp = 0.1
@parameters Reduction_Start_Time = 2010.0
@parameters Initial_Population = 3.369e9
@parameters Preindustrial_CO2 = 5.9e11
@parameters reference_emissions = 5.0e9
@parameters Utility_Coeff = 1.0
@parameters Rate_of_Time_Pref = 0.03
@parameters Reference_Temperature = 3.0
@parameters concentration_sensitivity = 0.0
@parameters Initial_Return_Trend = -0.00054
@parameters Capital_Elast_Output = 0.25
@parameters Depreciation_Rate = 0.065
@parameters Init_Fact_Prod_Growth_Rt = 0.015
@parameters Rate_of_Inequal_Aversion = 1.0
@parameters Red_Cost_Nonlinearity = 2.887
@parameters init_co2_intens_decline_rt = 0.01168
@parameters Init_Pop_Growth_Rate = 0.0223
@parameters Ref_Cons_per_Cap = 1000.0
@parameters smoothing_time = 20.0
@parameters Ultimate_Population = 1.0572e10
@parameters CO2_Rad_Force_Coeff = 4.1
@parameters init_CO2_intensity_of_output = 0.000519
@parameters Climate_Damage_Nonlinearity = 2.0
@parameters Fact_Prod_Gr_Rt_Dec_Rt = 0.011
@parameters CO2_Transfer_Time = 120.0
@parameters Heat_Trans_Coeff = 500.0
@parameters Base_Year = 1989.0
@parameters init_CO2_in_Atmos = 6.77e11
@parameters constant_reduction = 0.0
@parameters Initial_Capital = 1.6e13
@parameters Climate_Damage_Scale = 0.013
@parameters Heat_Capacity_Ratio = 0.44
@variables Deep_Ocean_Temp(t) = -1.0
@variables Pop_Growth_Rate(t) = -1.0
@variables CO2_Intens_Decline_Rt(t) = -1.0
@variables CO2_Intensity_of_Output(t) = -1.0
@variables Capital(t) = -1.0
@variables Atmos_UOcean_Temp(t) = -1.0
@variables CO2_in_Atmos(t) = -1.0
@variables Cum_Disc_Utility(t) = 0.0
@variables Population(t) = -1.0
@variables Fact_Prod_Growth_Rt(t) = -1.0
@variables Factor_Productivity(t) = 1.0
@variables Capital_Labor_Ratio(t)
@variables Capital_Life(t)
@variables Capital_Output_Ratio(t)
@variables Chg_A_UO_Temp(t)
@variables Chg_DO_Temp(t)
@variables Climate_Damage_Frac(t)
@variables Climate_Damages(t)
@variables Climate_Feedback_Param(t)
@variables CO2_Emiss(t)
@variables CO2_Intens_Capital(t)
@variables CO2_Intens_Dec_Rt_Decline_Rt(t)
@variables CO2_Net_Emiss(t)
@variables CO2_Rad_Forcing(t)
@variables CO2_Storage(t)
@variables TEMPVARSMOOTHED_Cons_Growth_Rate(t)
@variables Cons_Growth_Rate(t)
@variables Consumption(t)
@variables Consumption_per_Cap(t)
@variables Decline_CO2_Intens(t)
@variables Decline_Pop_Gr_Rt(t)
@variables Depreciation(t)
@variables Desired_Capital_Growth_Rate(t)
@variables Discount_Factor(t)
@variables Discounted_Utility(t)
@variables DO_Heat_Cap(t)
@variables Expected_Return(t)
@variables TEMPVARSMOOTHED_Expected_Return_Trend(t) = (Initial_Return_Trend)
@variables Expected_Return_Trend(t)
@variables Fact_Prod_Gr_Rt_Decline_Rt(t)
@variables Fact_Prod_Incr_Rt(t)
@variables Feedback_Cooling(t)
@variables GHG_Red_Cost_Frac(t)
@variables GHG_Reduction_Frac(t)
@variables Heat_Transfer(t)
@variables Indicated_Interest_Rate(t)
@variables Investment(t)
@variables Investment_Fraction(t)
@variables Labor_Output_Ratio(t)
@variables Marg_Prod_Capital(t)
@variables Marg_Prod_Carbon(t)
@variables Marg_Return_Capital(t)
@variables Net_CC_Impact(t)
@variables Net_Investment(t)
@variables Net_Pop_Incr(t)
@variables Net_Savings_Rate(t)
@variables Output(t)
@variables Pop_Gr_Rt_Decline_Rt(t)
@variables Radiative_Forcing(t)
@variables Rate_of_CO2_Transfer(t)
@variables Reduction_Costs(t)
@variables Reference_CO2_Emissions(t)
@variables Reference_Output(t)
@variables TEMPVARSMOOTHED_Smoothed_Return(t) = ((Marg_Return_Capital)-((Initial_Return_Trend)*(smoothing_time)))
@variables Smoothed_Return(t)
@variables Temp_Diff(t)
@variables Total_Pop_Utility(t)
@variables uncontrolled_emissions(t)
@variables Utility(t)



    #définition des equations:
    eqs = [
                D(Atmos_UOcean_Temp) ~ Chg_A_UO_Temp
                D(Capital) ~ Investment-Depreciation
                D(CO2_in_Atmos) ~ CO2_Net_Emiss-CO2_Storage
                D(CO2_Intens_Decline_Rt) ~ -CO2_Intens_Dec_Rt_Decline_Rt
                D(CO2_Intensity_of_Output) ~ -Decline_CO2_Intens
                D(Cum_Disc_Utility) ~ Discounted_Utility
                D(Deep_Ocean_Temp) ~ Chg_DO_Temp
                D(Fact_Prod_Growth_Rt) ~ -Fact_Prod_Gr_Rt_Decline_Rt
                D(Factor_Productivity) ~ Fact_Prod_Incr_Rt
                D(Pop_Growth_Rate) ~ -Decline_Pop_Gr_Rt
                D(Population) ~ Net_Pop_Incr
                Capital_Labor_Ratio ~ (Capital)/(Population)
                Capital_Life ~ (1)/(Depreciation_Rate)
                Capital_Output_Ratio ~ (Capital)/(Output)
                Chg_A_UO_Temp ~ ((Radiative_Forcing)-((Feedback_Cooling)-(Heat_Transfer)))/(A_UO_Heat_Cap)
                Chg_DO_Temp ~ (Heat_Transfer)/(DO_Heat_Cap)
                Climate_Damage_Frac ~ (1)/((1)+((Climate_Damage_Scale)*(((Atmos_UOcean_Temp)/(Reference_Temperature))^(Climate_Damage_Nonlinearity))))
                Climate_Damages ~ (Reference_Output)*((1)-(Climate_Damage_Frac))
                Climate_Feedback_Param ~ (CO2_Rad_Force_Coeff)/(Climate_Sensitivity)
                CO2_Emiss ~ ((1)-(GHG_Reduction_Frac))*((CO2_Intensity_of_Output)*(Output))
                CO2_Intens_Capital ~ (CO2_Emiss)/(Capital)
                CO2_Intens_Dec_Rt_Decline_Rt ~ (CO2_Intens_Decline_Rt)*(Fact_Prod_Gr_Rt_Dec_Rt)
                CO2_Net_Emiss ~ (Atmos_Retention)*(CO2_Emiss)
                CO2_Rad_Forcing ~ (CO2_Rad_Force_Coeff)*(log((2),((CO2_in_Atmos)/(Preindustrial_CO2))))
                CO2_Storage ~ ((CO2_in_Atmos)-(Preindustrial_CO2))*(Rate_of_CO2_Transfer)
                D(TEMPVARSMOOTHED_Cons_Growth_Rate) ~ ((Consumption_per_Cap)-TEMPVARSMOOTHED_Cons_Growth_Rate) /(TIME_STEP)
                Cons_Growth_Rate ~ (log((Consumption_per_Cap)/(TEMPVARSMOOTHED_Cons_Growth_Rate)))/(TIME_STEP)
                Consumption ~ (Output)-(Investment)
                Consumption_per_Cap ~ (Consumption)/(Population)
                Decline_CO2_Intens ~ (CO2_Intensity_of_Output)*(CO2_Intens_Decline_Rt)
                Decline_Pop_Gr_Rt ~ (Pop_Growth_Rate)*(Pop_Gr_Rt_Decline_Rt)
                Depreciation ~ (Capital)*(Depreciation_Rate)
                Desired_Capital_Growth_Rate ~ (((Expected_Return)-(Rate_of_Time_Pref))/(Rate_of_Inequal_Aversion))+(Pop_Growth_Rate)
                Discount_Factor ~ exp((-(Rate_of_Time_Pref))*((t)-(Base_Year)))
                Discounted_Utility ~ (Total_Pop_Utility)*(Discount_Factor)
                DO_Heat_Cap ~ (Heat_Capacity_Ratio)*(Heat_Trans_Coeff)
                Expected_Return ~ (Marg_Return_Capital)+((Expected_Return_Trend)*(Capital_Life))
                D(TEMPVARSMOOTHED_Expected_Return_Trend) ~ ((((Marg_Return_Capital)-(Smoothed_Return))/(smoothing_time))-TEMPVARSMOOTHED_Expected_Return_Trend) /(smoothing_time)
                Expected_Return_Trend ~ TEMPVARSMOOTHED_Expected_Return_Trend
                Fact_Prod_Gr_Rt_Decline_Rt ~ (Fact_Prod_Growth_Rt)*(Fact_Prod_Gr_Rt_Dec_Rt)
                Fact_Prod_Incr_Rt ~ (Factor_Productivity)*(Fact_Prod_Growth_Rt)
                Feedback_Cooling ~ (Atmos_UOcean_Temp)*(Climate_Feedback_Param)
                GHG_Red_Cost_Frac ~ (1)-((Red_Cost_Scale)*(IfElse.ifelse((GHG_Reduction_Frac)>(0),((GHG_Reduction_Frac)^(Red_Cost_Nonlinearity)),(0))))
                GHG_Reduction_Frac ~ ((IfElse.ifelse( t<(Reduction_Start_Time),0,(1)))*(max((0),(min((1),((constant_reduction)+(((concentration_sensitivity)*(((CO2_in_Atmos)-(Preindustrial_CO2))/(Preindustrial_CO2)))+((emissions_sensitivity)*(((uncontrolled_emissions)-(reference_emissions))/(reference_emissions))))))))))
                Heat_Transfer ~ (Temp_Diff)*((DO_Heat_Cap)/(Heat_Trans_Coeff))
                Indicated_Interest_Rate ~ (Rate_of_Time_Pref)+((Cons_Growth_Rate)*(Rate_of_Inequal_Aversion))
                Investment ~ min((Output),((Depreciation)+((Capital)*(Desired_Capital_Growth_Rate))))
                Investment_Fraction ~ (Investment)/(Output)
                Labor_Output_Ratio ~ (Population)/(Output)
                Marg_Prod_Capital ~ (Capital_Elast_Output)*((Output)/(Capital))
                Marg_Prod_Carbon ~ (Reference_Output)/((Reference_CO2_Emissions)*((Red_Cost_Scale)*((Red_Cost_Nonlinearity)*(IfElse.ifelse((GHG_Reduction_Frac)>(0),((GHG_Reduction_Frac)^((Red_Cost_Nonlinearity)-(1))),(0)))))) 
                Marg_Return_Capital ~ (Marg_Prod_Capital)-(Depreciation_Rate)
                Net_CC_Impact ~ (GHG_Red_Cost_Frac)*(Climate_Damage_Frac)
                Net_Investment ~ (Investment)-(Depreciation)
                Net_Pop_Incr ~ (Population)*(Pop_Growth_Rate)
                Net_Savings_Rate ~ (Net_Investment)/(Output)
                Output ~ (Reference_Output)*(Net_CC_Impact)
                Pop_Gr_Rt_Decline_Rt ~ (Init_Pop_Growth_Rate)/(log((Ultimate_Population)/(Initial_Population)))
                Radiative_Forcing ~ (CO2_Rad_Forcing)+(_tables[:Other_GHG_Rad_Forcing][((t)/(one_year))])
                Rate_of_CO2_Transfer ~ (1)/(CO2_Transfer_Time)
                Reduction_Costs ~ ((1)-(GHG_Red_Cost_Frac))*(Reference_Output)
                Reference_CO2_Emissions ~ (Reference_Output)*(CO2_Intensity_of_Output)
                Reference_Output ~ (Output_in_1965)*((Factor_Productivity)*((((Capital)/(Initial_Capital))^(Capital_Elast_Output))*(((Population)/(Initial_Population))^((1)-(Capital_Elast_Output)))))
                D(TEMPVARSMOOTHED_Smoothed_Return) ~ ((Marg_Return_Capital)-TEMPVARSMOOTHED_Smoothed_Return) /(smoothing_time)
                Smoothed_Return ~ TEMPVARSMOOTHED_Smoothed_Return
                Temp_Diff ~ (Atmos_UOcean_Temp)-(Deep_Ocean_Temp)
                Total_Pop_Utility ~ (Utility)*(Population)
                uncontrolled_emissions ~ (Reference_Output)*(CO2_Intensity_of_Output)
                Utility ~ (Utility_Coeff)*(IfElse.ifelse((Rate_of_Inequal_Aversion)==(1),(log((Consumption_per_Cap)/(Ref_Cons_per_Cap))),(((((Consumption_per_Cap)/(Ref_Cons_per_Cap))^((1)-(Rate_of_Inequal_Aversion)))-(1))/((1)-(Rate_of_Inequal_Aversion)))))


    ]

    #il faut maintenant définir le solveur:

    @named sys= ODESystem(eqs)
    sys= structural_simplify(sys)
    prob= ODEProblem(sys,[],(1965,2305))
    solved=solve(prob)
    using Plots

    plot(solved)